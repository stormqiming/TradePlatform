...
export default {
  ...
  // 组件挂载完毕，执行一次
  mounted() {
    // 在发请求之前，带给服务器参数【searchParams参数发生变化带给服务器】
    this.getData();
  },
  computed: {
    // mapGetters里面的写法传递的是数组，因为getters计算是没有划分模块的【没有home,search之分】
  ...
  methods: {
    ...
    // 删除分类的名字
    removeCategoryName() {
      // 把带给服务器的参数置空了，还要向服务器发请求
      // 带给服务器参数可有可无的，如果属性值为空的，字符串还是会把相应的字段带给服务器
      ...
      this.getData();
      // 地址栏也需要修改,进行路由的跳转(在当前的路由组件中跳自己)
      // 严谨：本意是删除query，如果路径当中出现了params，不应该删除，路由跳转的时候应该带着
    ...
    // 排序的操作
    changeOrder(flag) {
      // flag形参:是一个标记，代表用户点击的是综合(1)还是价格(2)【用户点击的时候传递过来的】
      // 获取原始的
  ...
  watch: {
    ...
    $route(newValue, oldValue) {
      ...
      // 再次发起ajax请求
      this.getData();
      // 每一次请求完毕，应该把相应的1，2，3级分类的id置空,让他接收下一次的相应的1，2，3级id
